...
/**
 * Facade class orchestrating the process of driver license approval submit from REST endpoint
 * by authorized sales manager and sending it to Camunda bypassing state of UserTask awaiting.
 *
...
@Service
@AllArgsConstructor
public class CamundaSendApproveDriverLicenseFacade {
    ...
    @Transactional
    public void approveDriverLicense(UUID requestForApprovalID, boolean isApproved) {
        ...
        CarOrderDto carOrderDto = carOrderService
                .approveDriverLicense(requestForApprovalDto.carOrder().getId(), isApproved);
